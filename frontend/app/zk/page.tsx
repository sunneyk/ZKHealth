...
type Proof = { proof_id: string; biomarker_name: string; threshold_display: string; passes: number; solana_tx_id: string; created_at: string };
type VerifyResult = { proof_valid: boolean; signature_valid: boolean; fully_verified: boolean; passes: boolean };
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="proof-name truncate">
            ...
            <span className="proof-threshold">{proof.threshold_display}</span>
          </p>
...
export default function ZkPage() {
  ...
  async function handleProve(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedObs || !threshold || !biomarkerName) return;
    ...
    try {
      const res = await fetch(`${API}/api/zk/prove`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ obs_id: selectedObs, threshold: t, biomarker_name: biomarkerName, anchor_on_chain: anchorOnChain }),
      });
      if (!res.ok) throw new Error((await res.json()).detail);
      const result = await res.json();
      toast.success(result.passes ? "Proof generated — claim passes ✓" : "Proof generated — claim does not pass");
      const updated = await fetch(`${API}/api/zk/list`).then(r => r.json());
      setProofs(updated);
      setSelectedObs(""); setThreshold("");
    } catch (err: unknown) {
        ...
        {observations.length === 0 ? (
          ...
          <div className="card">
            <form onSubmit={handleProve} className="space-y-4">
              ...
              <div>
                <label className="form-label">Threshold — prove value is below this</label>
                <input type="number" step="any" min="0.001" value={threshold}
                  onChange={e => setThreshold(e.target.value)} placeholder="e.g. 50" required className="form-input" />
              </div>
              ...
              <button type="submit" disabled={proving || !selectedObs || !threshold} className="btn-primary-md">
                {proving ? "Generating proof… (5–15s)" : "Generate ZK Proof"}
              </button>
            </form>
          </div>
        )}
